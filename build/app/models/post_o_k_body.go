...
type PostOKBody struct {
	...
	// gcp
	Gcp []*PostOKBodyGcpItems `json:"gcp"`
}
...
func (m *PostOKBody) validateGcp(formats strfmt.Registry) error {
	if swag.IsZero(m.Gcp) { // not required
		return nil
	}

	for i := 0; i < len(m.Gcp); i++ {
		if swag.IsZero(m.Gcp[i]) { // not required
			continue
		}

		if m.Gcp[i] != nil {
			if err := m.Gcp[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("gcp" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("gcp" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *PostOKBody) contextValidateGcp(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.Gcp); i++ {

		if m.Gcp[i] != nil {
			if err := m.Gcp[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("gcp" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("gcp" + "." + strconv.Itoa(i))
